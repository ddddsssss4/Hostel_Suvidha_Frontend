...
>>>>>>> f3ad0f53c6eedfdc3e3a1a5df4ec62c9350fee62
    ...
    const totalClothes = ['t-shirts', 'lower', 'shirts', 'bedsheet', 'pillow cover', 'shorts', 'pants', 'towel']
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/v1/students/laundry', {
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center">
                    ...
                    <div className="flex gap-2 items-center justify-end">
                      ...
                      <Field
                        name="date"
                        type="date"
                        className={`mt-1 block w-full lg:w-[40%] border-none rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-white bg-[#171A1C] p-2 placeholder-gray-400 ${errors.date && touched.date ? 'border-red-500' : ''}`}
                      />
          ...
          <div className="p-6  bg-[#202528] border-t-8 border-[#7380EC] lg:w-1/3 rounded-md space-y-6">
            ...
            <div className="card p-4 rounded-md bg-[#171A1C] flex flex-col">
              ...
              <div className="type mt-4 gap-2 flex flex-wrap">
                ...
                <div className="tiles h-10 py-1 px-2 gap-2 rounded-2xl flex bg-[#202528]">
                  Bedsheets
                  <div className="count p-1 w-8 h-8 text-center rounded-[40%] bg-[#7380EC]">1</div>
                </div>
              </div>
              <div className="status flex justify-between mt-2 items-center">
                <h2 className="font-bold text-md">STATUS :</h2>
            ...
            <div className="flex flex-col gap-4">
              {previousClothes.map((entry, index) => (
                <div key={index} className="card p-4 rounded-md bg-[#171A1C] flex flex-col">
                  <div className="head font-bold text-xl flex justify-between">
                  ...
                  <div className="type mt-4 gap-2 flex flex-wrap">
                    {Object.entries(entry.items).map(([key, value]) => (
                      value > 0 && key !== 'date' && (
                        <div key={key} className="tiles h-10 py-1 px-2 gap-2 rounded-2xl flex bg-[#202528]">
                          {key.charAt(0).toUpperCase() + key.slice(1)}
                          <div className="count p-1 w-8 h-8 text-center rounded-[40%] bg-[#7380EC]">{value}</div>
                        </div>
                      )
                    ))}
                  </div>
